...
async def test_perform_sentiment_analysis(client: AsyncClient):
    response = await client.post(
        "/api/v1/nlp/sentiment_analysis",
        json={"text": "I love sunny days but hate the rain."},
    )
...
async def test_perform_entity_recognition(client: AsyncClient):
    response = await client.post(
        "/api/v1/nlp/entity_recognition",
        json={"text": "London is a big city in the United Kingdom."},
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_perform_language_translation(client: AsyncClient):
    response = await client.post(
        "/api/v1/nlp/language_translation",
