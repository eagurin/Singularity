...
from app.models.schemas.nlp import (
    ...
    EntityRecognitionRequest, EntityRecognitionResponse,
    LanguageTranslationRequest, LanguageTranslationResponse,
    AdvancedNLPFeaturesRequest, AdvancedNLPFeaturesResponse
)
...
@router.post("/advanced_nlp_features", response_model=AdvancedNLPFeaturesResponse)
def perform_advanced_nlp_features(request: AdvancedNLPFeaturesRequest, db: Session = Depends(get_db)):
    """
    ...
    if request.target_language and request.target_language.strip():
        translation_result = nlp_service.translate_text(request.text, request.target_language)
    
    return AdvancedNLPFeaturesResponse(
        sentiment_analysis=sentiment_result.result,
        entity_recognition=entity_result.entities,
        language_translation=translation_result.translated_text if translation_result else None
    )
